...
int main() {
    ...
    auto proxy = sdbus::createProxy(std::move(permissionsName), std::move(permissionsObjectPath));


    auto get_system_time_impl = [&object, &proxy, &permissionsInterfaceName]() -> uint64_t {
        ...
        try {
            ...
            if (err.getName() == "org.freedesktop.DBus.Error.ServiceUnknown") {
                // THINK ABOUT IT : who is responsible for throwing/handling "Not Available" error?
                throw sdbus::Error(sdbus::Error::Name{"com.system.permissions.NotAvailable"},
                        "Permission server is not available");
            } else {
                throw sdbus::Error(sdbus::Error::Name{"com.system.time.InternalError"},
        ...
        if(!has_perm) {
            throw sdbus::Error(sdbus::Error::Name{"com.system.permissions.UnauthorizedAccess"},
    ...
    object->addVTable(
        sdbus::registerMethod("GetSystemTime").implementedAs(std::move(get_system_time_impl)))
