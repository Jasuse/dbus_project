...
PermissionsService::PermissionsService(sdbus::IConnection& connection, sdbus::ObjectPath path) :
    AdaptorInterfaces (connection, std::move(path)), db("permissions.db", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE) {
    RegisterAdaptor();
...
void PermissionsService::RequestPermission(sdbus::Result<>&& result, int permission) {
    auto msg = getObject().getCurrentlyProcessedMessage();
    std::thread(
        [this, result = std::move(result), permission = std::move(permission), msg = std::move(msg)] () {
            if (permission > 0) {
...
void PermissionsService::CheckApplicationHasPermission(sdbus::Result<bool>&& result, 
                                                    std::string path, int permission) {
    std::thread(
        [this, result = std::move(result), path = std::move(path), permission = std::move(permission)] () {
            if (permission > 0) {
                result.returnError(sdbus::Error(
                    sdbus::Error::Name{"com.system.permissions.InvalidPermission"},
                    "There is no such permission"));
