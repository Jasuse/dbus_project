...
bool check_permission_internal(const SQLite::Database& db,
...
int main() {
    ...
    std::mutex db_mutex;
    SQLite::Database db("permissions.db",
    ...
    auto check_permission_impl = [&permissions, &db_mutex, &db](
    ...
    permissions
        ->addVTable(sdbus::registerMethod("RequestPermission")
                        .implementedAs(std::move(request_perm_impl)),
                    sdbus::registerMethod("CheckApplicationHasPermission")
                        .implementedAs(std::move(check_permission_impl)))
